...
* `FORMAT_OUTPUT` - Output format of the vulnerabilities. Supported formats are `standard`, `json`, `table`, `slack`. Default is `standard`. If `JSON_OUTPUT` is set to true, this option is ignored.
...
* `SLACK_WEBHOOK_URL` - If using `slack` format output, set this to your Slack webhook address.
